...
inline const char *EnumNameDEVICE_COMMAND(DEVICE_COMMAND e) {
    if (::flatbuffers::IsOutRange(e, DEVICE_COMMAND_WRITE, DEVICE_COMMAND_EXIT)) {
        return "";
    }
...
public:
    tt_vcs_core() : x_(0), y_(0) {}
    tt_vcs_core(uint64_t _x, uint64_t _y) : x_(::flatbuffers::EndianScalar(_x)), y_(::flatbuffers::EndianScalar(_y)) {}
...
struct DeviceRequestResponse FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
    ...
    DEVICE_COMMAND command() const { return static_cast<DEVICE_COMMAND>(GetField<int8_t>(VT_COMMAND, 0)); }

    const ::flatbuffers::Vector<uint32_t> *data() const {
        return GetPointer<const ::flatbuffers::Vector<uint32_t> *>(VT_DATA);
    }

    const tt_vcs_core *core() const { return GetStruct<const tt_vcs_core *>(VT_CORE); }
    ...
    bool Verify(::flatbuffers::Verifier &verifier) const {
        return VerifyTableStart(verifier) && VerifyField<int8_t>(verifier, VT_COMMAND, 1) &&
               VerifyOffset(verifier, VT_DATA) && verifier.VerifyVector(data()) &&
               VerifyField<tt_vcs_core>(verifier, VT_CORE, 8) && VerifyField<uint64_t>(verifier, VT_ADDRESS, 8) &&
...
struct DeviceRequestResponseBuilder {
    ...
    void add_core(const tt_vcs_core *core) { fbb_.AddStruct(DeviceRequestResponse::VT_CORE, core); }

    void add_address(uint64_t address) { fbb_.AddElement<uint64_t>(DeviceRequestResponse::VT_ADDRESS, address, 0); }

    void add_size(uint32_t size) { fbb_.AddElement<uint32_t>(DeviceRequestResponse::VT_SIZE, size, 0); }

    explicit DeviceRequestResponseBuilder(::flatbuffers::FlatBufferBuilder &_fbb) : fbb_(_fbb) {
...
inline ::flatbuffers::Offset<DeviceRequestResponse> CreateDeviceRequestResponseDirect(
    ...
    return CreateDeviceRequestResponse(_fbb, command, data__, core, address, size);
}
...
inline bool VerifyDeviceRequestResponseBuffer(::flatbuffers::Verifier &verifier) {
    return verifier.VerifyBuffer<DeviceRequestResponse>(nullptr);
}

inline bool VerifySizePrefixedDeviceRequestResponseBuffer(::flatbuffers::Verifier &verifier) {
    return verifier.VerifySizePrefixedBuffer<DeviceRequestResponse>(nullptr);
}

inline void FinishDeviceRequestResponseBuffer(
    ::flatbuffers::FlatBufferBuilder &fbb, ::flatbuffers::Offset<DeviceRequestResponse> root) {
    fbb.Finish(root);
}
