...
static inline void dispatch_remote_transfer_command(
    tt_SiliconDevice& driver, remote_transfer_sample_t const& command, std::vector<uint32_t>& payload) {
    RemoteTransferType transfer_type = std::get<0>(command);
...
get_default_full_dram_dest_generator(int seed, tt_SiliconDevice* device) {
