...
std::set<chip_id_t> get_target_devices() {
    ...
    std::unique_ptr<tt_ClusterDescriptor> cluster_desc_uniq =
        tt_ClusterDescriptor::create_from_yaml(test_utils::GetAbsPath("blackhole_1chip_cluster.yaml"));
...
TEST(SiliconDriverBH, CreateDestroy) {
    ...
    for (int i = 0; i < 50; i++) {
        tt_SiliconDevice device = tt_SiliconDevice(
            ...
            test_utils::GetAbsPath("blackhole_1chip_cluster.yaml"),
            target_devices,
...
//     {
//         std::unique_ptr<tt_ClusterDescriptor> cluster_desc_uniq =
//         tt_ClusterDescriptor::create_from_yaml(test_utils::GetClusterDescYAML()); if
...
//     uint32_t num_host_mem_ch_per_mmio_device = 1;
//     tt_SiliconDevice device = tt_SiliconDevice("./tests/soc_descs/wormhole_b0_8x10.yaml",
//     test_utils::GetClusterDescYAML(), target_devices, num_host_mem_ch_per_mmio_device, false, true, true,
//     simulated_harvesting_masks); auto sdesc_per_chip = device.get_virtual_soc_descriptors();
...
//     uint32_t num_host_mem_ch_per_mmio_device = 1;
//     // Initialize the driver with a 1x1 descriptor and explictly do not perform harvesting
//     tt_SiliconDevice device = tt_SiliconDevice("./tests/soc_descs/wormhole_b0_1x1.yaml",
//     test_utils::GetClusterDescYAML(), target_devices, num_host_mem_ch_per_mmio_device, false, true, false,
//     simulated_harvesting_masks); auto sdesc_per_chip = device.get_virtual_soc_descriptors();
...
//     tt_SiliconDevice device = tt_SiliconDevice("./tests/soc_descs/wormhole_b0_8x10.yaml",
//     test_utils::GetClusterDescYAML(), target_devices, num_host_mem_ch_per_mmio_device, false, true, true,
//     simulated_harvesting_masks); set_params_for_remote_txn(device); auto mmio_devices =
        ...
        for (int loop = 0; loop < 100;
            ...
            for (auto& core : device.get_virtual_soc_descriptors().at(i).workers) {
                device.write_to_device(
                    ...
                    tt_cxy_pair(i, core),
                    address,
                    "DYNAMIC_TLB_BASE_INDEX");
                device.wait_for_non_mmio_flush();  // Barrier to ensure that all writes over ethernet were commited
                test_utils::read_data_from_device(
                    device, readback_vec, tt_cxy_pair(i, core), address, 40, "DYNAMIC_TLB_BASE_INDEX");
                ASSERT_EQ(vector_to_write, readback_vec)
                    << "Vector read back from core " << core.x << "-" << core.y << "does not match what was written";
                device.wait_for_non_mmio_flush();
                device.write_to_device(
    ...
    for (int i = 0; i < target_devices.size(); i++) {
        for (int loop = 0; loop < 100;
            ...
            for (int ch = 0; ch < NUM_CHANNELS; ch++) {
                ...
                device.wait_for_non_mmio_flush();  // Barrier to ensure that all writes over ethernet were commited
                test_utils::read_data_from_device(
                    device, readback_vec, tt_cxy_pair(i, subchan), address, 40, "DYNAMIC_TLB_BASE_INDEX");
